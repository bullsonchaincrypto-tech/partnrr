...
router.get('/csv/outreach', (req, res) => {
  const rows = queryAll(`
...
router.get('/csv/sponsors', (req, res) => {
  const rows = queryAll(`
...
router.get('/csv/ranking', (req, res) => {
  const rows = queryAll(`
...
router.get('/pdf/kontrakt/:outreachId', async (req, res) => {
  try {
    ...
    const kontrakt = queryOne('SELECT * FROM kontrakt WHERE outreach_id = ?', [msg.id]);
