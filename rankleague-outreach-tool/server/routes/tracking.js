...
export function createTrackingId(outreachId, type = 'influencer') {
...
export function getTrackingPixelHtml(trackingId, serverUrl) {
...
router.get('/pixel/:trackingId', (req, res) => {
  ...
  const record = queryOne('SELECT * FROM email_tracking WHERE tracking_id = ?', [trackingId]);
  if (record) {
    runSql(
    ...
    if (!record.oppnad) {
      if (record.outreach_id) {
        const msg = queryOne('SELECT status FROM outreach_meddelanden WHERE id = ?', [record.outreach_id]);
...
router.get('/message/:outreachId', (req, res) => {
  const record = queryOne('SELECT * FROM email_tracking WHERE outreach_id = ?', [Number(req.params.outreachId)]);
